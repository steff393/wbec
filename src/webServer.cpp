...
void webServer_setup() {
	...
	server.on("/json", HTTP_GET, [](AsyncWebServerRequest *request) {
		...
		for (int i = from; i < to; i++) {
			...
			data[F("box")][i][F("version")]  = String(mb_version(i), HEX);
			data[F("box")][i][F("chgStat")]  = mb_status(i);
			data[F("box")][i][F("currL1")]   = mb_amperages(i)[0];
			data[F("box")][i][F("currL2")]   = mb_amperages(i)[1];
			data[F("box")][i][F("currL3")]   = mb_amperages(i)[2];
			data[F("box")][i][F("pcbTemp")]  = mb_temperature(i);
			data[F("box")][i][F("voltL1")]   = mb_voltages(i)[0];
			data[F("box")][i][F("voltL2")]   = mb_voltages(i)[1];
			data[F("box")][i][F("voltL3")]   = mb_voltages(i)[2];
			data[F("box")][i][F("extLock")]  = mb_extLock(i);
			data[F("box")][i][F("power")]    = mb_power(i);
			data[F("box")][i][F("energyP")]  = mb_energyPowerOn(i) / 1000.0f;
			data[F("box")][i][F("energyI")]  = mb_energyTotal(i) / 1000.0f;
			data[F("box")][i][F("energyC")]  = goE_getEnergySincePlugged(i) / 1000.0f;
			data[F("box")][i][F("currMax")]  = mb_amperageMaximum(i);
			data[F("box")][i][F("currMin")]  = mb_amperageMinimum(i);
			char txt[65]; mb_getAscii(i, 17, 32, txt);
			data[F("box")][i][F("logStr")]   = txt;
			data[F("box")][i][F("wdTmOut")]  = mb_watchdogTimeout(i);
			data[F("box")][i][F("standby")]  = mb_standby(i);
			data[F("box")][i][F("remLock")]  = mb_remLock(i);
			data[F("box")][i][F("currLim")]  = mb_amperageLimit(i);
			data[F("box")][i][F("currFs")]   = mb_amperageFailsafe(i);
	...
	server.on("/pv", HTTP_GET, [](AsyncWebServerRequest *request) {
		...
		data[F("box")][F("chgStat")]  = mb_status(id);
		data[F("box")][F("power")]    = mb_power(id);
		data[F("box")][F("currLim")]  = mb_amperageLimit(id);
